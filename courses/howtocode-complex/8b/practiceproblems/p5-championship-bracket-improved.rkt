  ...
  (cond [(false? br) false]
        [else
         (if (string=? t (bracket-team-lost br))
             (bracket-team-won br)
             (local [(define match-br-won (ouster t (bracket-br-won br)))]
               (if (not (false? match-br-won))
                   match-br-won
                   (ouster t (bracket-br-lost br)))))]))

;; Justification:
;; ouster is going recursively into bracket-br-won twice.
;; Once to see if going into the won bracket returns something other than false
;; If it does return something other than false, it goes back into the branch again to retreive the value.
